  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="address in addressInfo" :key="address.id">
        <span class="username" :class="{ selected: address.isDefault == 1 }"
          @click="changeDefault(address, addressInfo)">{{ address.consignee }}</span>
      ...
      <div class="detail">
        ...
        <ul class="list clearFix" v-for="order in orderInfo.detailArrayList" :key="order.skuId">
          <li>
            <img :src="order.imgUrl" height="100px">
          </li>
          <li>
            <p>
              {{ order.skuName }}
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ userDefaultInfo.fullAddress }}</span>
        收货人：<span>{{ userDefaultInfo.fullAddress }}</span>
        <span>{{ userDefaultInfo.phoneNum }}</span>
      </div>
    </div>
    <div class="sub clearFix">
      <q class="subBtn" @click="submitOrder">提交订单</q>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async submitOrder() {
      let data = {
        consignee: this.userDefaultInfo.consignee,
        consigneeTel: this.userDefaultInfo.phoneNum,
        deliveryAddress: this.userDefaultInfo.fullAddress,
        paymentWay: 'ONLINE',
        orderComment: this.message,
        orderDetailList: this.orderInfo.detailArrayList
      }
      try {
        await this.$store.dispatch('trade/submitOrder', { tradeNo: this.orderInfo.tradeNo, data })
